...
namespace debug_handlers
{
    bool handle_set_position_sensorfusion_stop_relocalize_test()
    {
        ...
        position_sensorfusion_pipeline::set_stop_relocalize_test_enabled(enabled);

        if (enabled == true)
        {
            return handler_helpers::write_response_text("position_sensorfusion_stop_relocalize_test enabled");
        }

        return handler_helpers::write_response_text("position_sensorfusion_stop_relocalize_test disabled");
    }
}
