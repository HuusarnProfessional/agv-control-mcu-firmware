...
namespace debug_handlers
{
    bool handle_set_position_sensorfusion_bypass()
    {
        ...
        position_sensorfusion_pipeline::set_bypass_offset_fusion(enabled);

        if (enabled == true)
        {
            return handler_helpers::write_response_text("position_sensorfusion_bypass enabled");
        }

        return handler_helpers::write_response_text("position_sensorfusion_bypass disabled");
    }
}
