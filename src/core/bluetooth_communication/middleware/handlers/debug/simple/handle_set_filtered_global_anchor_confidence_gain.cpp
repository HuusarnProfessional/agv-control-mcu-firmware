...
namespace debug_handlers
{
    bool handle_set_filtered_global_anchor_confidence_gain()
    {
        ...
        if (filtered_global_position::set_candidate_anchor_heading_confidence_gain_permille(gain_permille) == false)
        {
            return handler_helpers::write_response_text("filtered_global_anchor_confidence_gain rejected");
        }
        ...
        const int formatted_length = std::snprintf(
            response,
            sizeof(response),
            "filtered_global_anchor_confidence_gain %u",
            static_cast<unsigned>(filtered_global_position::get_candidate_anchor_heading_confidence_gain_permille()));
