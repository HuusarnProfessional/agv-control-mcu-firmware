...
namespace debug_handlers
{
    bool handle_set_position_sensorfusion_local_only()
    {
        ...
        position_sensorfusion_pipeline::set_local_only_mode(enabled);

        if (enabled == true)
        {
            return handler_helpers::write_response_text("position_sensorfusion_local_only enabled");
        }

        return handler_helpers::write_response_text("position_sensorfusion_local_only disabled");
    }
}
