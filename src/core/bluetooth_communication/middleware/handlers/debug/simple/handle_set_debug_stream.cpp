...
namespace debug_handlers
{
    bool handle_set_debug_stream()
    {
        ...
        if (middleware_parse_helpers::read_until_comma_or_end(stream_name, sizeof(stream_name), ended, timeout_us) == false)
        {
            return false;
        }
        ...
        if (middleware_parse_helpers::read_bool_and_end(is_enabled, timeout_us) == false)
        {
            return handler_helpers::write_response_text("err bad_format");
        }
