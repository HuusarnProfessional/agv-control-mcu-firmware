...
namespace incoming_request_handlers
{
    bool handle_get_position_sensorfusion()
    {
        ...
        const position_sensorfusion::output_snapshot fused_position = position_sensorfusion::read_output();
        const std::int32_t x_mm = static_cast<std::int32_t>(fused_position.x_um / 1000LL);
        const std::int32_t y_mm = static_cast<std::int32_t>(fused_position.y_um / 1000LL);
        const double heading_deg = static_cast<double>(fused_position.heading_urad) * 180.0 / 3141592.653589793;
        std::int32_t confidence_position = 0;
        std::int32_t confidence_heading = 0;

        if (fused_position.has_pose == true)
        {
            std::uint16_t confidence_position_clamped = fused_position.confidence_position;
            std::uint16_t confidence_heading_clamped = fused_position.confidence_heading;
        ...
        const int formatted_length = std::snprintf(
            response,
            sizeof(response),
            "rsp:position_sensorfusion(%d,%d,%.2f,%d,%d,%u,%u)",
            static_cast<int>(x_mm),
            static_cast<int>(y_mm),
            heading_deg,
            static_cast<int>(confidence_position),
            static_cast<int>(confidence_heading),
            static_cast<unsigned int>(fused_position.pose_id),
            static_cast<unsigned int>(fused_position.branch_id));
